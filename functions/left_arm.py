...
shoulder_z = height*1.2 + min_z
waist_z = height*0.67 + min_z
hand_x = height* 0.15 / 2
# print("Waist is at the value : " + str(chest_z))
...
bpy.ops.object.mode_set(mode='EDIT')
bpy.ops.mesh.bisect(plane_co=(19,19, shoulder_z), plane_no=(0, 0, 1), clear_inner=True, clear_outer=False)
bpy.ops.mesh.bisect(plane_co=(hand_x, 1, 1),plane_no=(1, 0, 0), clear_inner=False, clear_outer=True)
bpy.ops.mesh.bisect(plane_co=(hand_x, 1, 1),plane_no=(0, 1, 0), clear_inner=False, clear_outer=True)
bpy.ops.object.mode_set(mode='OBJECT')
...
for i in range(len(vertices)):
    if vertices[i][2] - waist_z < 1e-8:
        chest_vertices.append(vertices[i][2])

print(len(chest_vertices))





print("Chesbt inches: " + str(len(chest_vertices) *8.984380013604919e-05 ))
