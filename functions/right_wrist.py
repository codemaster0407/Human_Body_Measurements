def right_wrist(path, object_name):
    ...
    bpy.ops.object.mode_set(mode='EDIT')
    wrist_height = -0.05425
    #For right hand bisection
    bpy.ops.mesh.bisect(plane_co=(-0.25,0,wrist_height), plane_no=(1, 0, 0), clear_inner=False, clear_outer=True)

    bpy.ops.mesh.select_all(action = "SELECT")

    bpy.ops.mesh.bisect(plane_co=(0,0,wrist_height), plane_no=(0, 0, 1), clear_inner=False, clear_outer=True)
    #bpy.ops.object.mode_set(mode='OBJECT')
